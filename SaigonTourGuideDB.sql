-- ============================================================
-- SCHEMA: Hệ thống Quản lý & Gợi ý Địa điểm Du lịch TP.HCM
-- Lưu ý: PostgreSQL mặc định encode UTF-8 -> lưu tiếng Việt có dấu
-- bình thường bằng VARCHAR/TEXT, KHÔNG cần NVARCHAR (đó là kiểu SQL Server)
-- DATETIME không tồn tại trong PostgreSQL -> dùng TIMESTAMP
--
-- LƯU Ý QUAN TRỌNG: "User" là từ khóa dành riêng (reserved keyword) trong
-- PostgreSQL. Để giữ tên bảng là "User" (không đổi thành "users"), BẮT BUỘC
-- phải quote bằng dấu ngoặc kép "User" ở MỌI nơi tham chiếu - cả lúc tạo bảng,
-- lúc khai báo FK ở bảng khác, và sau này khi viết SELECT/INSERT/UPDATE.
-- Quên quote 1 chỗ là sẽ lỗi cú pháp ngay.
-- ============================================================
...
CREATE TABLE "User" (
    ...
    avatar              VARCHAR(200),             -- giữ nguyên 1 cột như bạn viết, nullable vì không bắt buộc có ảnh
    email               VARCHAR(100) NOT NULL UNIQUE,  -- thêm UNIQUE: email không được trùng giữa các user
    phone               VARCHAR(15),              -- bỏ NOT NULL, tăng độ dài để chứa mã vùng +84
    date_of_birth       DATE,                     -- bỏ NOT NULL: cho phép bổ sung sau
    gender              VARCHAR(7),               -- bỏ NOT NULL: thông tin nhạy cảm, không nên ép buộc
...
CREATE TABLE Place (
    ...
    CONSTRAINT chk_status CHECK (status IN ('active', 'pending', 'closed'))
);
...
CREATE TABLE PlaceTag (
    ...
    FOREIGN KEY (place_id) REFERENCES Place(id) ON DELETE CASCADE,
    FOREIGN KEY (tag_id)   REFERENCES InterestTag(id) ON DELETE CASCADE,
    CONSTRAINT chk_relevance CHECK (relevance BETWEEN 0.0 AND 1.0)
);
...
CREATE TABLE PlaceAgeGroup (
    ...
    CONSTRAINT chk_age_group CHECK (age_group IN ('children', 'teenager', 'young_adult', 'adult', 'middle_age', 'senior')),
    CONSTRAINT chk_suitability CHECK (suitability BETWEEN 1 AND 5)
);
...
CREATE TABLE VisitedPlace (
    ...
    CONSTRAINT chk_source CHECK (source IN ('manual', 'review'))
);
...
CREATE TABLE ItineraryItem (
    ...
    CONSTRAINT chk_transport CHECK (transport_mode IN ('walk', 'bike', 'motor', 'car', 'bus', 'grab'))
);
